...
import { RefreshCw, Clock, CheckCircle2, AlertTriangle, Trophy, ListTodo, Sparkles, Upload, FileText, Loader2, X } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-fade-in max-w-full overflow-x-hidden">
      ...
      {/* Import tasks from a meeting transcript */}
      <ImportTranscriptCard />
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-surface-500 dark:text-surface-400 mt-0.5">
            Upload a meeting transcript (.docx or .txt, up to 2 files). AI reads it and drafts tasks assigned to the best-matched people. Review them in the{' '}
            <Link href="/inbox" className="text-brand-600 hover:underline font-medium">Task Inbox</Link>.
          </p>
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            {result !== null && result > 0 && (
              <Link href="/inbox" className="text-sm text-brand-600 hover:underline font-medium">
                Review {result} draft{result === 1 ? '' : 's'} →
              </Link>
            )}
          </div>
        </div>
